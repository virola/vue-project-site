<template>
  <div>
    <section class="container user-page">
      ...
      <div class="user-link">
        <router-link :to="{path: '/password', query: {redirectTo: redirectTo}}" class="link-active">忘记密码</router-link>
      </div>
      <form novalidate action="#" class="needs-validation" :class="{'was-validated': validation.form}" v-on:submit.prevent="onSubmit">
        ...
        <div class="form-group">
          <input type="submit" value="确认修改" class="form-control btn btn-primary">
        </div>
        <div class="alert alert-danger" role="alert" v-if="formMsg">
          {{formMsg}}
        </div>
        <p class="center">
          <a href="#" @click.prevent="$router.back">返回</a>
        </p>
      </form>
    </section>
